...
ruta_archivo_local <- "/home/sanma_ch/buckets/b1/datasets/competencia_f03.csv.gz"
...
for( sem in PARAM$train_final$semillas ) {
  ...
  if( !file.exists( arch_modelo ) ) {
    ...
    # Se elimina el rm(modelito) aquí para que la variable quede disponible
    # para el cálculo de importancia
    
  } else {
    # Bloque ELSE: El modelo SÍ existe, lo cargo
    cat(paste0("✅ Modelo encontrado para semilla: ", sem, ". Cargando...\n"))
    modelito <- lgb.load(arch_modelo)
  }
  # 
  # # 2. CÁLCULO DE IMPORTANCIA (Se ejecuta siempre, pues 'modelito' está definido)
  # tb_importancia <- as.data.table(lgb.importance(modelito))
  # 
  # # 3. GUARDADO DEL ARCHIVO DE IMPORTANCIA
  # archivo_importancia <- paste0("./modelitos/impo_", sem, ".txt")
  # 
  ...
  # 
  # 4. LIMPIEZA: Ahora se puede liberar la memoria de 'modelito'
  rm(modelito)
  gc()
}
