...
```{r read_data}
# leo el dataset
#dataset <- fread("competencia_f03.csv.gz" )
...
PARAM <- list()
PARAM$experimento <- "e3-001"
PARAM$semilla_primigenia <- 254881

plocal <- list()

# 501
plocal$qcanaritos <- 50L
...
ruta_archivo_local <- "/home/gustavoaldana838/buckets/b1/datasets/competencia_f03.csv.gz"
...
MESES_A_ANALIZAR <- generar_rango_meses(201901, 202109)
...
# Contar la cantidad de registros donde foto_mes es 202109
cantidad_registros <- dataset[foto_mes == 202109, .N]
...
# 1. IDENTIFICACIÓN DE COLUMNAS MONETARIAS


# Usamos grep para encontrar todas las columnas que empiezan con 'm' (montos)
# El patrón "^m" significa 'comienza con m'.
columnas_monetarias <- grep("^m", names(dataset), value = TRUE)

cat("Columnas monetarias identificadas para la sumatoria:\n")
print(columnas_monetarias)



# 2. CREACIÓN DE LAS NUEVAS VARIABLES


cat("\n⏳ Creando variables 'sum_signo' y 'sum_abs'...\n")

# A. SUMA CON SIGNO (Suma Neta o Saldo Neto)
# Se usa rowSums() sobre las columnas seleccionadas.
# na.rm = TRUE es crucial para sumar valores incluso si faltan datos en alguna columna
dataset[, sum_signo := rowSums(.SD, na.rm = TRUE), .SDcols = columnas_monetarias]


# B. SUMA ABSOLUTA (Actividad Bruta o Volumen Total)
# 1. Se crea una matriz temporal con los valores absolutos de las columnas monetarias
matriz_abs <- data.matrix(dataset[, ..columnas_monetarias])
matriz_abs <- abs(matriz_abs)

# 2. Se calcula la suma de filas sobre esta matriz de valores absolutos
dataset[, sum_abs := rowSums(matriz_abs, na.rm = TRUE)]

# 3. Limpiar la matriz temporal
rm(matriz_abs)
gc()

cat("✅ Variables 'sum_signo' y 'sum_abs' creadas exitosamente.\n")


# 3. CREACIÓN DE VARIABLES DE RATIO (NORMALIZACIÓN)

DIVISORAS <- c("cliente_edad", "cliente_antiguedad", "ctrx_quarter", "ctrx_quarter_normalizado")
VARIABLES_SUMA <- c("sum_signo", "sum_abs")

cat("\n⏳ Creando variables de ratio (divididas por edad, antiguedad, etc.)...\n")

for (suma_var in VARIABLES_SUMA) {
  for (div_var in DIVISORAS) {
    new_var_name <- paste0(suma_var, "_div_", div_var)
    
    # Se calcula la nueva variable por referencia.
    # Nota: R maneja la división por cero asignando Inf (o -Inf si el numerador es negativo)
    # o NaN (si el numerador es 0). Esto es usualmente aceptable en Machine Learning.
    dataset[, (new_var_name) := get(suma_var) / get(div_var)]
    
    cat(paste0("   - Creada: ", new_var_name, "\n"))
  }
}

cat("✅ 8 variables de ratio creadas exitosamente.\n")


# 4. VERIFICACIÓN (Opcional)

# Lista de todas las nuevas variables creadas
nuevas_vars <- c("sum_signo", "sum_abs")
for (suma_var in VARIABLES_SUMA) {
  for (div_var in DIVISORAS) {
    nuevas_vars <- c(nuevas_vars, paste0(suma_var, "_div_", div_var))
  }
}

cat("\nVista previa de las nuevas variables:\n")
# Muestra un resumen de las sumatorias y los nuevos ratios
print(head(dataset[, .SD, .SDcols = c("numero_de_cliente", "foto_mes", nuevas_vars)], 5))
...
```{r guardo_dataset, eval=FALSE}
fwrite( dataset,
    file =  "competencia_lag_03.csv.gz",
    sep = ","
)
...
# La ruta local DEBE ser la ruta de Linux que corresponde al archivo en el bucket.
# Intentaremos la ruta directa, asumiendo que el bucket se monta en /home/ger_samartino/buckets/
ruta_archivo_local <- "competencia_lag_03.csv.gz"
...
PARAM$train_final$lgbm$param_completo <-  list(
  ...
  num_iterations= 999L, # dejo libre la cantidad de arboles, zLightGBM se detiene solo
...
for( sem in PARAM$train_final$semillas ) {
  ...
  archivo_importancia <- paste0("./modelitos/impo_", sem, ".txt") # Definir aquí
  
  # 0. VERIFICACIÓN RÁPIDA: Si el archivo de IMPORTANCIA ya existe, saltamos la semilla
  if( file.exists( archivo_importancia ) ) {
      cat(paste0("⏭️ Importancia ya calculada para semilla: ", sem, ". Saltando...\n"))
      next # Pasa a la siguiente semilla del bucle
  }
  
  
  # 1. VERIFICACIÓN Y CARGA/ENTRENAMIENTO
  if( !file.exists( arch_modelo ) ) {
    # Bloque IF: El modelo NO existe, lo entreno y lo guardo
    
    cat(paste0("⏳ Entrenando nuevo modelo para semilla: ", sem, "\n"))
    ...
    # Bloque ELSE: El modelo SÍ existe, lo cargo (necesario para el paso 2)
    cat(paste0("✅ Modelo encontrado para semilla: ", sem, ". Cargando...\n"))
    modelito <- lgb.load(arch_modelo)
  }
  
  # 2. CÁLCULO DE IMPORTANCIA (Solo si el 'next' no se ejecutó)
  tb_importancia <- as.data.table(lgb.importance(modelito))
  
  # 3. GUARDADO DEL ARCHIVO DE IMPORTANCIA
  ...
  cat(paste0("   Importancia guardada en: ", basename(archivo_importancia), "\n"))
...
if( file.exists("prediccion3.txt") )
  file.remove("prediccion3.txt")
...
tb_prediccion <- fread("prediccion3.txt")
